...
<body class="is-preload">
    ...
    <!-- 新增產品視窗 -->
        ...
        <div class="popup-content">
            ...
            <!-- 圖片上傳區塊 -->
            <div class="action-row">
            <label for="add-product-pic">上傳圖片：</label>
            <input type="file" id="add-product-pic" name="add-product-pic" accept="image/*">
            </div>
    ...
    <!-- 修改彈出視窗 -->
        <div id="edit-popup" class="popuple">
            <div class="popup-content">
                ...
                <!-- 圖片上傳欄位 -->
                <div class="action-row">
                    <label for="edit-pic">上傳圖片：</label>
                    <input type="file" id="edit-pic" name="edit-pic" accept="image/*">
                </div>
    ...
    <!-- 火鍋類商品修改彈窗 -->
        ...
        <div class="popup-content">
            ...
            <div class="action-row">
            <label for="hotpot-edit-pic">上傳圖片：</label>
            <input type="file" id="hotpot-edit-pic" name="hotpot-edit-pic" accept="image/*" />
            </div>
    ...
    <!-- 新增火鍋視窗 -->
        <div id="add-hotpot-popup" class="popuple" style="display:none;">
            <div class="popup-content">
                ...
                <!-- 圖片上傳區塊（火鍋專用） -->
                <div class="action-row">
                <label for="add-hotpot-pic">上傳圖片：</label>
                <input type="file" id="add-hotpot-pic" name="add-hotpot-pic" accept="image/*">
                </div>
    ...
    <!-- 分類 -->
        ...
        // 取得不重複分類
        $sqlCategories = "SELECT DISTINCT category FROM menu ORDER BY category";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                if (!empty($row["pic"])) {
                    ...
                    echo '<img src="' . htmlspecialchars($row["pic"]) . '" alt="' . htmlspecialchars($row["Menu_name"]) . ' 圖片">';
                    echo '</div>';
                }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        button.addEventListener('click', () => {
            ...
            cards.forEach(card => {
            const cardCategory = card.querySelector('.category-label').textContent.replace('分類：', '').trim();
    ...
    // 新增產品add_menu_and_ingredient.php
        document.getElementById("add-product-confirm-button").addEventListener("click", async function () {
            ...
            const picInput = document.getElementById("add-product-pic");
            ...
            try {
                const resp = await fetch("add_menu_and_ingredient.php", {
                    method: "POST",
                    body: formData  // 不設定 Content-Type，瀏覽器會自動設定 multipart/form-data
                });
    ...
    // 新增火鍋add_hotpot.php
        ...
        document.getElementById("add-hotpot-confirm-button").addEventListener("click", function(event) {
            ...
            const hotpotName = document.getElementById("add-hotpot-name").value.trim();
            const hotpotPrice = document.getElementById("add-hotpot-price").value.trim();
            const picInput = document.getElementById("add-hotpot-pic");
            const picFile = picInput.files[0];

            if (!hotpotName || !hotpotPrice) {
                alert("請輸入品項名稱與價格！");
                return;
            }
            if (!picFile) {
            ...
            // 建立 FormData
            const formData = new FormData();
            formData.append("hotpotName", hotpotName);
            formData.append("hotpotPrice", hotpotPrice);
            formData.append("pic", picFile);
            ...
            // 不用設定 Content-Type，瀏覽器會自動設定 multipart/form-data

            xhr.onload = function() {
                if (xhr.status === 200) {
                    alert(xhr.responseText); // 後端直接輸出訊息
                    location.reload();
                } else {
                    alert("新增失敗，請稍後再試");
                }
    ...
    // 修改視窗edit_menu.php(可以修改但圖片修改上會發生更新後的圖片命名問題)
        ...
        const editPicInput = document.getElementById("edit-pic");
        ...
        document.querySelectorAll(".edit-button").forEach(function (btn) {
            ...
            editPicInput.value = ""; // 清空檔案欄

            // 紀錄初始值，方便後續比較
            initialData = { menuId, name, category, price };
            ...
            // 也綁定 menuId 給確認按鈕
            editConfirmButton.setAttribute("data-menu-id", menuId);
            });
        });
    ...
    // 火鍋修改(填入當前該項商品內容)
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            const addRowBtn = document.getElementById("edit-add-ingredient-row-button");
            ...
            function createIngredientRow(selectedId = "", quantity = "") {
                ...
                selectName.name = "ingredient-select[]";

                window.ingredientOptions.forEach(function(opt) {
                    const option = document.createElement("option");
                    option.value = opt.Ingredient_ID;
                    option.textContent = opt.Ingredient_name;
                    selectName.appendChild(option);
                });

                if (selectedId) {
                    selectName.value = selectedId;
                }
                ...
                removeBtn.className = "remove-ingredient-button";
            ...
            document.querySelectorAll(".edit-button").forEach(function (btn) {
                btn.addEventListener("click", function (e) {
                    ...
                    // 清空表格body（先移除所有 tr，保留最後一列加入品項按鈕行）
                    ingredientsTableBody.innerHTML = "";

                    // 用料列先依資料產生
                    ingredients.forEach(function (ing) {
                        const row = createIngredientRow(ing.Ingredient_ID, ing.quantity);
