...
<body class="is-preload">
    ...
    // 新增商品
        ...
        addProductConfirmBtn.addEventListener("click", () => {
            ...
            // 提交表單資料（使用 Fetch 或 AJAX 來處理新增資料）
            const data = {
        ...
        addHotpotConfirmBtn.addEventListener("click", () => {
            ...
            const hotpotPrice = parseFloat(document.getElementById("add-hotpot-price").value);

            if (!hotpotName || isNaN(hotpotPrice)) {
            alert("請填寫完整的火鍋資料！");
            return;
            }

            // 提交火鍋資料（使用 Fetch 或 AJAX 來處理新增資料）
            const data = {
            hotpot_name: hotpotName,
            price: hotpotPrice
            };

            fetch('add_hotpot.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
